...
public class ArtistPage extends BasePage<ArtistPage> {
    ...
    @Override
    @Step("Проверка, что страница художника загружена")
    ...
    public ArtistPage checkBiography(String expectedText) {
        biography.shouldHave(text(expectedText));
        return this;
    }

    @Step("Проверить имя художника '{expectedText}'")
    public ArtistPage checkArtistName(String expectedText) {
        artistName.shouldHave(text(expectedText));
        return this;
    }

    @Step("Проверить изображение аватара")
    public ArtistPage checkAvatarImg(String expectedBase64Image) {
